...
- [x] **#467** Transport resilience so the series COMPLETES (no partial reports). A dropped/refused opponent connection on a single move — even the last move of sub-game 6 — used to abort the whole game and send no email. `RemoteMoveClient` now retries each move up to 4× with a fresh connection (2 s backoff); a transient blip recovers. If the opponent server is **sustained-down** (all attempts fail) it raises `OpponentUnreachableError`, and `ChallengeRunner._sub_outcome` **forfeits** that sub-game to us (Cop@home → cop_wins, Thief@away → thief_wins) so the game always completes 6/6 and the full §9.2 email is sent. They agreed to throw, so the forfeit matches reality; `mutual_agreement` stays null.
- [x] **#468** `bonus_claim` now follows the ex06 rubric — **win=10, loss=7, tie=5** — derived from `totals_by_group` via `claim_from_totals` (was a hardcoded 10/10, which wrongly claimed 10/10 on a tie). Applies to every emit path (panel, inbound observer, `cop_thief.report`).
- [x] **#469** Frozen-opponent timeout + UI production submit. (a) `fetch_remote_move` now wraps connect+call in a 20 s deadline (`asyncio.wait_for`), so a FROZEN opponent (connects but never replies) raises instead of hanging → retries (2×) → `OpponentUnreachableError` → the runner forfeits that sub-game to us and the game finishes. (b) Panel has a **PRODUCTION SUBMIT** checkbox: when ticked, `_email` builds the reporter with an UNLOCKED guard (`SubmissionSafetyGuard(locked=False)`), so the examiner address typed in REPORT EMAIL sends without the safety block — no config edit/restart needed. Sender stays the authenticated burner account.
