...
- [x] **#459** COSMOS77 match prep — their rules (`match_setup/COSMOS77_RULES.txt`) adopt ours; reconciled in `COSMOS77_REPLY.txt`. Added **deterministic tournament mode** (`game.deterministic_moves`: move = pure fn(observation), greedy minimax, no exploration/history → byte-agreeing replays) and a **barriers on/off** toggle (`game.barriers_enabled`). Guard `_ALWAYS_ALLOW` is now the config burner (was the banned hardcoded address).
