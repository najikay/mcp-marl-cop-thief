...
| **SEC-06** | **Dispute log archive.** Each inter-group game seals an immutable evidence bundle (`data/archive/`): every transmission (both sides) with its board snapshot + `board_sha256` + hostility verdict, the final report, an evidence summary, and a `bundle_sha256` over the whole record. Any later edit breaks the seal, so it is verifiable proof for lecturer adjudication if the opponent cheats and disputes the result; the seal hash is surfaced in the emailed report (`dispute_bundle_sha256`). |
...
| **STRAT-01** | **Barrier-move mechanic (ex06 §4.3).** A Cop `[INTENT: BARRIER]` walls the cell it **vacates** and steps to an adjacent free cell — impassable to both, **no agent ever stands on a barrier**, budget ≤ 5/sub-game, Thief never. The minimax planner chooses *when* to spend a wall (it's in the search action set, one barrier-move per legal step), driving multi-step herding toward `thief_trapped`. |
